...
// Fassung   : V01-00
// Stand     : 2026-09-22
// Herkunft  : P08_ToDo-Ticket, src/storage/speicherwerk.ts (Arbeitspunkt 2,
//             dort erprobt). Neu aufgesetzt auf B04-C01 V02-00, B04-C04
//             V02-00, B04-C05 V01-01 und B04-C08 V01-00, statt - wie in
//             Arbeitspunkt 2 - Teile davon selbst nachzubauen.
...
export class Speicherwerk<T extends Datenbestand> {
  ...
  private readonly einDurchgang: () => Promise<void>;
  ...
  constructor(
    ...
    // ausgeloest wird.
    this.einDurchgang = nacheinander(() => this.lauf());
  }
  ...
  async jetztSpeichern(): Promise<void> {
    ...
    await this.einDurchgang();
  }

  /** Uebernimmt einen vom Benutzer entschiedenen Stand als verbindlich -
   *  schreibt unbedingt, ohne erneuten Abgleich. */
  async uebernehmeEntscheidung(db: T): Promise<void> {
    this.neuesterStand = db;
    this.cache.ausstehendSchreiben(this.optionen.datei, db);
    try {
      await this.speicher.schreiben(db, this.optionen.datei, this.optionen.ordner);
      this.gespeicherterCloudStand = await this.speicher.standHolen(this.optionen.datei, this.optionen.ordner);
      this.gespeicherterStand = db;
      this.cache.baselineSchreiben(this.optionen.datei, db);
      this.cache.ausstehendLoeschen(this.optionen.datei);
      this.setze("gespeichert", "");
    } catch (error) {
